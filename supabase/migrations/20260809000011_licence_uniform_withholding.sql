-- Correction to 20260809000010: a withheld vintage was still leaking.
--
-- Caught by invariant I9, run as `anon` rather than as the owner. A premise with
-- no 2020 observation came back `observed = false` — "not surveyed that year" —
-- while a premise that had one came back withheld. Both statements are about a
-- dataset we may not redistribute, and the difference between them lets a reader
-- infer which premises it contains simply by watching where the answer changes.
--
-- So the rule is now uniform: for a vintage we may not redistribute, an
-- anonymous caller learns nothing — not the content, not the absence, not the
-- existence. `observed` is null, `withheld` is true, and the reason says so.
--
-- Third iteration on this one function, each one found by exercising the
-- anonymous path. The privileged path passed every time.
