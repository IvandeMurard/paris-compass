-- `confidence_reason` is a French sentence, built to be read by a person. Nothing
-- machine-checkable can key off it without matching a substring of prose — which is
-- exactly what scripts/eval/run.ts has done so far (`reason_contains`), and it is as
-- fragile as it sounds: a rewording breaks a passing case for no behavioural reason,
-- and a real regression can slip through if the new sentence happens to share the
-- same substring.
--
-- `confidence_rule` adds a small closed vocabulary, one code per CASE branch that
-- already exists in `confidence_reason` below — nothing new is decided, the same
-- branch now also emits a stable identifier next to its sentence. An enum rather
-- than text for the same reason `compass_confidence` is one: a typo becomes a
-- migration error instead of a silent new value nothing checks for.
--
-- PLAN.md §6.8, first of its three named gaps.

create type public.compass_confidence_rule as enum (
  'vintage_licence_withheld',
  'not_observed',
  'ordre_reattributed',
  'observed_matched',
  'shared_address',
...
comment on function public.compass_address_timeline is
  ...
  'branches, a stable code instead of a sentence.';
