...
mesurés dans `docs/tickets/w1-terrasses.md`. **Remesuré le 26 août** avant de reprendre le
ticket : 24 199 lignes, cinq de moins en vingt-quatre heures, et un champ `periode_installation`
que la section du 25 août disait absent — nul sur 97 % des lignes, mais existant. Les deux écarts
sont datés dans le ticket plutôt que réécrits par-dessus.
...
| `w1-terrasses` | 1 | Q3 2026 | Terrasses et étalages autorisés | La fiche restauration affiche oui/non/inconnu terrasse, avec le type (permanente, estivale) et la source. — **ingéré le 25/08**, démontré en appel anonyme sur les trois états, rattaché par adresse après avoir mesuré et rejeté le rattachement par proximité (un tiers de mauvais commerce sur un échantillon nommé) ; **critère atteint le 26/08** : la fiche du local rend les trois états dans le navigateur, avec le type, la réserve obligatoire et la date de la source. Détail dans `docs/tickets/w1-terrasses.md`. |
...
- **Un défaut trouvé en chemin, hors périmètre — corrigé depuis :** sur un millésime `retail_only`, `compass_address_timeline` concluait « plus un commerce » à partir de millésimes qu'elle retient dans la même réponse. `DIAGNOSTIC.md` §15, issue [#54](https://github.com/IvandeMurard/paris-compass/issues/54) `w0-conclusion` — **fait le 26 août**, migration `20260826000001`, invariants I29 à I31. La question que le ticket laissait ouverte a été tranchée par la mesure et non par le goût : la phrase était fausse pour l'appelant privilégié aussi, sur 18 647 des 24 573 locaux concernés.
...
- **Fait le 25 août 2026**, démontré par appel anonyme réel. Halles et Mail rendent chacun **deux** survies : le local (BDCom, **retenu** à l'anonyme — licence APUR non lue) et l'exploitant (SIRENE, Licence Ouverte v2, servi) — 55,1 % sur 185 aux Halles, 57,3 % sur 124 au Mail, cohorte 2017-01 → 2020-08 censurée à six ans. Contre-test privilégié : 86,5 % (268/310) et 86,7 % (137/158) côté local. **Quatre chiffres de ce bloc étaient faux**, remesurés avant d'écrire : « aucune source nouvelle » (le stock INSEE **est** une source nouvelle — la tranche géolocalisée ne porte ni date ni état, mesuré en base), « par tronçon » (1 tronçon sur 6 338 atteint n≥30 ; le grain est le quartier), « un café » au sens strict (n=18 au Mail — le grain est niv18 « Café et Restaurant »), et le couple Halles/Mail lui-même, qui **ne discrimine pas** côté BDCom (86,5 contre 86,7, Paris entier à 86,5) — Belleville 77,2 % contre Bel-Air 96,1 % est démontré à côté. Le fait produit : **le local persiste, l'exploitant tourne** (86,5 % contre 52,5 % à Paris entier). Détail complet dans `docs/tickets/w1-survie.md`. Même chantier que `docs/PLAN.md` §5.2 **et** §6.2.
...
- **Ingéré le 25 août**, mesuré contre le distant en appel anonyme, sur les trois états : 86 RUE ABBE GROULT (adresse non partagée) rend `terrasse_status: oui`, `terrasse_permanente: true` ; 7 RUE ABBE DE L'EPEE (adresse partagée par plusieurs locaux) rend `inconnu` plutôt qu'un local choisi au hasard ; 1 RUE ABBAYE rend `non`. Le rattachement par proximité, mesuré avant d'être écrit (médiane 4,4 m), a été rejeté après un contrôle sur 12 terrasses nommées : un tiers pointait sur le mauvais commerce. Rattaché par adresse à la place, en réutilisant la clé de rue déjà posée pour BODACC.
- **Fait le 26 août** : le « Fait quand » porte sur la fiche, et le front-end ne lisait aucun des quatre champs — c'est pour cela que `#15` restait ouverte. `src/i18n/terrasseText.ts` (21 tests) décide les quatre états et leurs phrases ; `PremiseHistorySheet` les affiche. Démontré dans le navigateur, clé publiable seule : 59 RUE GRENETA rend **Oui** avec « terrasse permanente », 72 RUE MONTORGUEIL (POKAWA) rend **Inconnu** avec « étalage » et « la source ne dit pas lequel détient l'autorisation », 71 RUE GRENETA rend **Non** — les trois avec la date de la source, lue de `compass_source_freshness` et non figée en dur. Source revérifiée le 26 : 24 199 lignes (contre 24 204 la veille), ODbL, `periode_installation` existe mais est nul à 97 % — la section du 25 août disait « aucun champ de date », c'est corrigé et daté. Le motif d'adresse du chargeur a été corrigé dans la foulée (suffixes collés au numéro : 24 154 adresses parsées contre 24 140, 14 gagnées, 0 perdue) et la couche rechargée pour le poser : quatre locaux répondent désormais autre chose que « non », et `source_as_of` passe au 26 août. Détail complet dans `docs/tickets/w1-terrasses.md`.
...
| Filosofi carroyé 200 m | INSEE | ingérée — migration posée et chargée le 8/09/2026 (2 170 carreaux, millésime 2021) | Licence Ouverte 2.0 (`lov2`, mesurée le 8/09/2026) | carreau 200 m | Revenu et population sur une maille qui sépare deux rues. | L'IRIS est trop large — ne pas s'en contenter. **Ne publie aucune médiane à cette maille** : seule une somme winsorisée existe (`ind_snv`), d'où une MOYENNE estimée par habitant, jamais une médiane — voir la migration `20260908000001` et `docs/tickets/w2-filosofi.md`. |
...
| Cœur de scoring | interdit dans src/core | Garder Measured<T>, les quatre niveaux, le gate (37 invariants, 24 baselines, 8 chronologies — remesurés le 2 septembre 2026). | Faire écrire scoring.ts par un LLM. RAG sur forums, Google reviews, SeLoger. |
...
| #62 | — | P1 | `compass_premises_within` consomme 70 % du budget `anon` de 3 s au rayon maximal — **ouvert le 27 août** en mesurant #61, hors de son périmètre. Ce n'est plus la porte qui est exposée mais la carte. **Fait le 28 août**, par la piste 2 : le compte cesse de matérialiser les cinq jointures de libellés, qui sont reportées sur les lignes que le `limit` garde, et l'index de `premise_observation` devient couvrant. 195 422 → 94 065 pages au rayon maximal ; `total_matched` inchangé sur 490 comparaisons. Mesuré à froid avant correction : le premier appel après dix heures d'inactivité était **annulé** (`57014`), pas seulement cher. La piste 3 — baisser le rayon maximal — est **refusée** comme une promesse produit retirée, décision écrite dans `docs/REPRISE.md`. Le même défaut, en pire, a été trouvé et corrigé dans `compass_bodacc_within` (380 695 → 146 596). Le **bras E** de la porte le rend mécanique. Détail dans `DIAGNOSTIC.md` §27 |
| #64 | — | P1 | `compass_street_rotation` et `compass_scoring_context_within`, les deux fonctions de rayon que #62 n'a pas corrigées — **ouvert le 28 août** en le corrigeant, hors périmètre. `street_rotation` est désormais la plus chère des quatre : 286 744 pages, 2 091 ms au premier appel observé. Les deux ne sont pas le même cas — l'une rend une ligne par local, donc son coût est sa réponse ; l'autre est un agrégat qui lit 64 147 lignes pour en rendre 2 609, donc il y a de la place. Une décision à écrire par fonction |
