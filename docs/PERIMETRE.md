...
- **La porte d'évaluation.** **Quarante et un** invariants, vingt-quatre baselines, huit cas dorés, l'historique de composition — *remesurés le 5 septembre 2026 par `grep -c '^-- @invariant ' eval/invariants.sql`. Ce paragraphe portait trente-sept, compté le 31 août ; quatre sont arrivés depuis — `I38` par `w1-catalogue`, `I39` à `I41` par `w1-observabilite`. Il avait annoncé quinze avant cela, un chiffre recopié devenu faux en silence : c'est le troisième recomptage, et c'est pourquoi la commande est écrite à côté du nombre.* C'est le seul actif qui s'accumule vraiment, et c'est le bon type d'accumulation : il ne rend pas les réponses plus fines, il rend une régression impossible à livrer en silence. **Corpus de non-régression, pas jeu d'entraînement** — la confusion entre les deux est précisément ce qui ferait dériver le produit.
