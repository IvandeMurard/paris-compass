...
- **La porte d'évaluation.** **Quarante-deux** invariants, vingt-quatre baselines, huit cas dorés, l'historique de composition — *remesurés le 6 septembre 2026 par `grep -c '^-- @invariant ' eval/invariants.sql`. Ce paragraphe portait quarante et un, compté le 5 septembre, et il était déjà faux le jour même : `I42` est arrivé avec `w1-geometrie` (#68). Il portait trente-sept le 31 août ; cinq sont arrivés depuis — `I38` par `w1-catalogue`, `I39` à `I41` par `w1-observabilite`, `I42` par `w1-geometrie`. Il avait annoncé quinze avant cela, un chiffre recopié devenu faux en silence : c'est le troisième recomptage, et c'est pourquoi la commande est écrite à côté du nombre.* C'est le seul actif qui s'accumule vraiment, et c'est le bon type d'accumulation : il ne rend pas les réponses plus fines, il rend une régression impossible à livrer en silence. **Corpus de non-régression, pas jeu d'entraînement** — la confusion entre les deux est précisément ce qui ferait dériver le produit.
...
cliquet asymétrique, où le corpus ne peut que grossir. À quarante et un invariants et près de trois
