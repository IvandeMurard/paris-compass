...
27. **Republier depuis Lovable, et c'est la seule chose que le dépôt ne peut pas faire seul.**
    Mesuré le 17 septembre 2026 par `w1-servi-contenu` (#217) : la production sert un bundle
    **antérieur à `w6-contexte` (#119)**, donc `/carte` et `/contexte/` ne sont pas déclarés et
    la fiche rend une 404. Deux bras le disent indépendamment — `servi` sur le texte du bundle,
    `page` sur le DOM rendu — et les deux resteront **rouges chaque matin** jusqu'à la
    republication, à juste titre. `DIAGNOSTIC.md` §60. Après republication : rejouer
    `npm.cmd run servi` et `npm.cmd run page` ; s'ils ne reverdissent pas, la cause est chez
    Lovable et c'est un ticket là-bas.
