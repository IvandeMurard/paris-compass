...
15. **`ticket/w6-analyse` attend une seule commande, et elle n'est pas dans mes mains.** Ouvert le
    6 septembre 2026 au soir. La branche porte `w6-analyse` (#50) en entier — la migration
    `20260906000001_analyses_du_schema.sql`, les invariants `I43` à `I46`, trois budgets de bras E,
    `DIAGNOSTIC.md` §40 et §41. **Elle est commitée, poussée, et volontairement NON fusionnée.**

    Ce qui manque : `supabase db push`. Le classifieur de permissions l'a refusé deux fois
    (Bash puis PowerShell), et il n'a pas été contourné — appliquer le SQL à la main laisserait
    le ledger non tenu, ce que `w1-ledger` (#82) existe pour attraper.

    ```powershell
    & "$env:USERPROFILE\supabase-cli\supabase.exe" db push --linked
    ```

    **Ne pas fusionner avant de l'avoir lancé.** `I43` à `I46` appellent quatre fonctions qui
    n'existent pas encore sur le distant : fusionner mettrait `eval` au rouge le lendemain matin,
    et `ledger` sortirait en 3 sur une migration suivie et non posée.

    Une fois posée, dans cet ordre : `npm.cmd run eval` (les quatre invariants et les trois
    budgets), la démonstration aux Halles en commentaire de #50, la fermeture de #50,
    `npm.cmd run sessions`, puis la fusion. **La table d'ordre est juste tant que #50 est
    ouverte** — `sessions:check` sortait 0 le 6 septembre au soir, 51 tickets et 24 fermés.

    Le SQL n'est pas un pari : syntaxe, comportement anonyme et privilégié, `I43` à `I46` au
    vert et les budgets ont tous été éprouvés contre le distant **en transaction annulée**, le
    6 septembre. Ce que la transaction ne peut pas donner, c'est le ledger.

    **Une revue est due sur cette branche** — migration et invariants, deux des quatre signes de
    « La revue » (`docs/SESSIONS.md`), par une session distincte.
