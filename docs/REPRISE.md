...
27. **Pointer le build Lovable sur le bon projet Supabase — le site est à jour et muet.**
    Remesuré le 17 septembre 2026 après la republication : le code servi est celui de `main`
    (`servi` sortie 0, 1 168 jetons sur 1 168, §60 clos), mais **le bundle publié vise
    `nwnhhvogwrzstslxtxca`**, l'ancien projet Lovable Cloud, qui n'a pas le schéma de Compass —
    404 `PGRST202` sur `compass_premises_within` là où `dbefhvmyfmmhjeetdddu` rend 200. La fiche
    rend donc un refus sur les quatre axes, en 1 535 ms, et se comporte correctement en le
    faisant. **Corriger `VITE_SUPABASE_URL` et `VITE_SUPABASE_PUBLISHABLE_KEY` dans
    l'environnement de build Lovable**, republier, rejouer `npm.cmd run porte:publie` (doit
    sortir en 0) et `npm.cmd run page` (doit rendre un **verdict**, pas un refus).
    `DIAGNOSTIC.md` §61. Le dépôt ne peut rien faire de plus : `.env` porte déjà la bonne valeur
    et est suivi par git depuis le §32 ; c'est l'environnement du build qui prime sur lui.

    **Et le bras qui l'a vu a raison mais parle mal** : `porte:publie` dit « n'a pas eu sa
    configuration » quand la vérité est « en a une, pour un autre projet ». C'est
    [#127](https://github.com/IvandeMurard/paris-compass/issues/127), ouverte avant que le cas
    n'existe.

    **Et une décision plus petite, du même relevé** : `src/i18n/survivalText.ts` porte douze
    chaînes de prose qu'aucun import n'atteint. Brancher le bloc ou retirer le module —
    `DIAGNOSTIC.md` §58. Tant que rien n'est tranché, le fichier reste hors de la population du
    quatorzième bras, ce qui est correct mais ne règle rien.

26. **`w2-rythme` (#208), fermée le 17 septembre 2026, est passée à `docs/REPRISE-ARCHIVE.md`**
    le même jour — par `w1-servi-contenu` (#217), pour tenir le budget de `documents.test.ts`.
    Le relevé entier y est ; le vivant est dans `docs/tickets/w2-rythme.md`, section « Livré ».
    Ce qu'elle laisse ouvert : `DIAGNOSTIC.md` §57, les deux `comment on` du distant
