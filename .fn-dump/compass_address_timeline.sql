CREATE OR REPLACE FUNCTION public.compass_address_timeline(p_location_id bigint)
 RETURNS TABLE(occurred_on date, granularity text, source text, source_ref text, source_url text, source_licence text, kind text, observed boolean, withheld boolean, activity_code text, label text, detail text, amount_eur numeric, evidence text, confidence compass_confidence, confidence_rule compass_confidence_rule, confidence_reason text)
 LANGUAGE sql
